...
% Single address.
% ---------------
\name{ P. Rajan$^{1}$ P. Burlina$^{1,2}$ M. Chen$^{2}$ B. Jedynak$^{3}$  N. Mehta$^{2}$ A. Sinha$^{4}$ G. Hager$^{1}$ D. Edell$^{2}$
...
\State Form the logical intersection image from all the binary detection images, then perform logical differencing between each detection image and the common intersection image.
...
\State Find the connected components in the difference images, filter out hollow objects based on connected component area and extent to produce a set of candidate moving objects on each image.
