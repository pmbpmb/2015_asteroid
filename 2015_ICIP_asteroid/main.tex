...
\begin{algorithm}[H]
\small
\caption{Trajectory Linking Algorithm for a triplet of images}
\label{algo:Trajectory_Linking}
\begin{algorithmic}[1]
\State Form the set of all trajectories connecting the detections from the first and last frame.
\State Construct the 2d-tree for the difference image corresponding to  image-2. (TODO: Mention bounding box.) 
\ForAll{trajectories connecting the first and last frames}	
\State Find the point of intersection of each trajectory with the middle frame.
\State Perform range search on the 2d-tree for image-2 to find a list of points within a small radius around this point of intersection.
\State Perform PCA on this subset of trajectories. Find the ratio of eigenvalues, $\lambda_{1}/(\lambda_{1}+ \lambda_{2})$. Reject trajectories for which this ratio exceeds a threshold. (TODO: Clarify this step)
\EndFor
\end{algorithmic}
\end{algorithm}
\vspace{-0.3cm}
...
\vspace{-0.3cm}
\section{CONCLUSION}
Detecting smaller {\em city-killer} asteroids in space-based surveys is important, as these  are not easily seen in earth-based surveys. We describe a pipeline for on-board NEO detection and characterize its performance on simulated space-based as well as real imagery and show that quadruplets offer much higher performance with little incidence on complexity. Performance results and preliminary computational requirements are promising with regard to the possible future deployment of this algorithm on proxy spacecraft processors such as an MCP750. 
