...
\begin{center}$
\begin{array}{@{\hspace{-0.7em}}c@{\hspace{0.2em}}c@{\hspace{0.2em}}}
\includegraphics[width=0.25\textwidth]{Figures/ROC_StarAsteroid_130_pchip_f20_NoTitle_LW15.pdf} &\includegraphics[width=0.25\textwidth]{Figures/ROC_Triplets_130_pchip_f20_NoTitle_LW15.pdf} 
\end{array}$
\end{center}
\vspace{-0.8cm}
...
Algorithmic development and results shown earlier were performed in MATLAB. The pipeline is being jointly implemented in C++ for benchmarking on a MCP750, a good proxy for the RAD750. The MCP750,  with clock speed of 367 MHz, is completely dedicated to the algorithm and not servicing spacecraft or instrument commands during benchmarking.  Run time for steps 2-8 (minus the 2d-trees) on a quadruplet is about one second. Preliminary tests indicate that 16 MB of RAM is used and the current executable is 1.3 MB. Any preprocessing such as step 1 would be implemented on an FPGA. While further optimization is under way, current results indicate that this algorithm can be deployed on a spacecraft processor.
