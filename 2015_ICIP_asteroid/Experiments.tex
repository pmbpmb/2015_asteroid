...
Results shown  earlier and the original pipeline were developed and tested in MATLAB. The pipeline is being jointly implemented in C++ for benchmarking on a MCP750 with clock speed of 367 MHz, a good proxy for the RAD750, an operational spacecraft processor. The MCP750 is completely dedicated to the algorithm and not servicing spacecraft or instrument commands during benchmarking.  Run time for steps 2-8 (minus the 2d-trees) is a little over one second. Preliminary tests indicate that 16 MB of RAM is used and the current RTP executable is 1.3 MB. Any preprocessing such as step 1 would be implemented on an FPGA. While further optimization is under way, current results indicate that this algorithm can be deployed on a spacecraft processor.
